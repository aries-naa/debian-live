...
do_netsetup ()
{
	...
	do
        ...
        # !!! alex !!!
        # ethtool=eth0:100:full:off,eth1:::on
        if [ -x /sbin/ethtool ]; then
            for x in $(cat /proc/cmdline); do
                case $x in
                    ethtool=*)
                        net_parameters=${x#ethtool=}
                        echo -n $net_parameters | awk -v RS="," -F : '{
eth="ethtool -s "$1
if ($2 != "") eth=eth " speed "$2
if ($3 != "") eth=eth " duplex "$3
if ($4 != "") eth=eth " autoneg "$4
system(eth)
}'
                        ;;
                esac
